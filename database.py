...
DATABASE = os.path.join(os.path.dirname(__file__), 'registration.db')

def get_db():
    """
    Get a database connection for the current request.
    Uses Flask's g object to ensure each request gets
    its own connection (connection pool pattern).
    """
    if 'db' not in g:
...
def init_app(app):
    """
    Register the close_db function with the Flask application.
    """
